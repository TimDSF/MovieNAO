...
class Recognition():
	...
	def recognize(self, timeoutCount = 5000):
			...
			for _ in range(timeoutCount):
				...
				try:
					if val[0][1] == 1 and b == 0:
						b = 1
						self.record.recordStart("/home/nao/test.wav")
					elif val[0][1] == 0 and b == 1:
						b = 0
						self.record.recordStop()
						self.sftp.download(self.IP, 22, self.usr, self.pas, "/home/nao/test.wav", "misc/audio.wav")
						rcd = 1
						break
				except:
					continue
		...
		if rcd == 0:
			...
			rst = self.aip.asr(get_file_content("misc/audio.wav"), "wav", 16000, {'dev_pid': 1536,})
